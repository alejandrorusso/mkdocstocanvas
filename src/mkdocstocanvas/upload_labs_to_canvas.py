...
def create_or_update_assignment(lab_name, html_content, points_possible=100):
    ...
    if existing_assignment:
        ...
        result = make_api_request(
            "PUT", f"/courses/{COURSE_ID}/assignments/{assignment_id}", json=update_data
        )
        if result:
            print(f"✓ Successfully updated assignment content: {lab_name}")
            print(f"  URL: {BASE_URL}/courses/{COURSE_ID}/assignments/{assignment_id}")
            print(
                f"  Note: Other settings (deadline, points, visibility) were preserved"
            )
            return True
    else:
        ...
        result = make_api_request(
            "POST", f"/courses/{COURSE_ID}/assignments", json=create_data
        )
        if result:
...
def find_lab_files():
    ...
    def lab_sort_key(path):
        ...
        match = re.search(r"Lab\s*(\d+)", filename, re.IGNORECASE)
...
def get_all_assignments():
    ...
    assignments = make_api_request(
        "GET", f"/courses/{COURSE_ID}/assignments?per_page=100"
    )
...
def delete_lab_assignments():
    ...
    for assignment in lab_assignments:
        ...
        response = make_api_request(
            "DELETE", f'/courses/{COURSE_ID}/assignments/{assignment["id"]}'
        )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Canvas Lab Assignment Management Script"
    )
    parser.add_argument(
        "--delete", action="store_true", help="Delete all lab assignments from Canvas"
    )
