...
def upload_image_to_canvas(
    image_path: Path, canvas_base_url: str, canvas_api_token: str, course_id: str
) -> Optional[str]:
...
def process_excel_macros(content: str, markdown_file_path: Path) -> str:
    ...
    def replace_excel_macro(match):
        ...
        # .resolve() will handle normpath like behavior and make it absolute
        excel_path = excel_path.resolve()

        print(
            f"  Rendering Excel sheet: {excel_path}"
    ...
    def replace_image(match):
        ...
        if full_image_path.exists():
            ...
            if canvas_url:
                ...
                print(f"✗ Failed to upload: {full_image_path.name}")
...
def resolve_internal_links(
    content: str, all_pages_info: List[Dict], course_id: str, canvas_base_url: str
) -> str:
    ...
    for page_info in all_pages_info:
        ...
        if "laplace" in page_title.lower() and "mechanism" in page_title.lower():
            ...
            "differential privacy" in page_title.lower() or "pure" in page_title.lower()
        ):
        ...
        if len(url_parts) >= 2:
            ...
            main_part = (
                "-".join(url_parts[1:-1]) if len(url_parts) > 2 else url_parts[0]
            )
    ...
    def replace_link(match):
        ...
        if base_url.endswith(".md"):
            ...
            page_slug = Path(link_url).stem.replace("_", "-").lower()
...
#     course_id: str,
#     all_pages_info: List[Dict] = None,
# ) -> str:
def process_markdown_to_html(
    md_page: MarkdownPage,
    all_pages_info: List[Dict] | None = None,
) -> str:
    """
    ...
    # if all_pages_info:
    #     html_content = resolve_internal_links(
    #         html_content, all_pages_info, course_id, canvas_base_url
    #     )
